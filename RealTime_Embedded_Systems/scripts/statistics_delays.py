...
def get_recording_data(directory_path):
    ...
    csv_files = glob.glob(os.path.join(directory_path, '*_statistics.csv'))
    ...
    for csv_file in csv_files:
        ...
        if 'Write Delay' in trade_df.columns and 'Minute Timestamp' in trade_df.columns:
            # Append the Timestamp and Write Delay columns to their respective lists
            timestamps.extend(trade_df['Minute Timestamp'].to_numpy())
...
def calculate_statistics(values):
    nan_indices = np.isnan(values)  # Get boolean array for NaN indices
    if np.any(nan_indices):  # If any NaN values are found
        print("NAN FOUND at indices:\n")
        for idx in np.where(nan_indices)[0]:  # Get indices of NaN values
...
def plot_combined_histogram(overall_values, first_half_values, second_half_values):
    ...
    plt.hist(overall_values, bins=num_bins, range=(0, 3000), edgecolor='black', alpha=0.5, density=True, label='Total Distribution')
    plt.hist(first_half_values, bins=num_bins, range=(0, 3000), edgecolor='blue', alpha=0.7, density=True, label='First 24-Hour Distribution')
    plt.hist(second_half_values, bins=num_bins, range=(0, 3000), edgecolor='red', alpha=0.7, density=True, label='Second 24-Hour Distribution')
...
if __name__ == "__main__":
    ...
    directory_path = "/home/gseli/Documents/uni-collection/UniversityCollection/RealTime_Embedded_Systems/build/csv"
